...
uint32_t ORCP::create_packet(uint8_t IR, uint8_t IT, uint8_t SC, uint8_t SS, uint8_t* data, uint32_t data_size)
{
	...
	out_buffer.size += CP_PACKET_HEADER_LENGTH; //4
	...
	// CRC
	uint16_t crc = orv::crypto::crc16(out_buffer.data, out_buffer.size);
	out_buffer.data[out_buffer.size] = (uint8_t)(crc >> 8);
	out_buffer.data[out_buffer.size + 1] = (uint8_t)(0xFF & crc);
	out_buffer.size += CP_PACKET_CRC_LENGTH; //2

	return out_buffer.size;
}

// send packet (docs/pp-protocol.txt)
...
int ORCP::parse(uint8_t *buf, uint32_t buf_size)
{
	...
	// check CRC
	input_packet.crc = (input_message.data[input_message.size-2] << 8) | input_message.data[input_message.size-1];
	
	uint16_t mes_crc = orv::crypto::crc16(input_message.data, input_message.size-CP_PACKET_CRC_LENGTH);
